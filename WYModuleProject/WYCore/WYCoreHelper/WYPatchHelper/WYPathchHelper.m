...
-(void)mcUpdateAllLocalPatchFiles
{
    ...
    WeakObj(self);
    [self.mcLocalPatchs enumerateObjectsUsingBlock:^(WYPathchModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        StrongObj(self);
 ...
 *  @param patch 补丁模型
 */
- (void) mcUpdatePatchFile: (WYPathchModel *) patch
{
 ...
 *
 */
- (void) mcInstallPatch: (WYPathchModel *)patch
{
    // 仅安装为"未安装状态的"补丁.
 ...
 */
-(BOOL)existWithArray:(NSArray *)array model:(WYPathchModel *)model
{
    BOOL result=NO;
    if (array.count==0) {
        return result;
    ...
    for (WYPathchModel *item in array) {
 ...
 *  @param model 对象
 */
-(BOOL)deleteFileWithJiaPathchModel:(WYPathchModel *)model
{
    if (!model) {
        return NO;
    }
 ...
 *  @return 补丁的本地存储路径.
 */
- (NSString *) mcPathForModel: (WYPathchModel *) model
{
...
- (void)setMcPatchs:(NSMutableArray *)mcPatchs {
    ...
    //先处理要删除的文件
    for (WYPathchModel *item in _mcLocalPatchs) {
    ...
    //接着对新补丁进行写入
    for (WYPathchModel *item in mcPatchs) {
